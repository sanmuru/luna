...
internal abstract class SourceWriter : AbstractFileWriter
{
    ...
    public virtual void WriteInternal()
    {
        ...
        WriteLine("namespace {0}.Syntax.InternalSyntax", this.RootNamespace);
        OpenBlock();
        WriteLine();
    ...
    {
        WriteFileHeader();
        WriteLine("namespace {0}.Syntax", this.RootNamespace);
        OpenBlock();
        WriteLine();
        this.WriteRedTypes();
        CloseBlock();
    ...
    {
        WriteFileHeader();
        WriteLine("namespace {0}", this.RootNamespace);
        OpenBlock();
        WriteLine("using System.Diagnostics.CodeAnalysis;");
    ...
    protected virtual void WriteGreenTypes()
    {
    ...
    protected virtual void WriteGreenType(TreeType node)
    {
    ...
    protected virtual void WriteGreenNodeConstructorArgs(List<Field> nodeFields, List<Field> valueFields)
    {
        foreach (var field in nodeFields)
        {
    ...
    protected virtual void WriteGreenSerialization(Node node)
    {
    ...
    protected virtual string GetWriterMethod(string type)
        => type switch
        {
            "bool" => "WriteBoolean",
    ...
    protected virtual string GetReaderMethod(string type)
        => type switch
        {
            "bool" => "ReadBoolean",
            _ => throw new InvalidOperationException($"Type '{type}' not supported for object reader serialization."),
    ...
    protected virtual void WriteCtorBody(List<Field> valueFields, List<Field> nodeFields)
    {
        // constructor body
        WriteLine($"this.SlotCount = {nodeFields.Count};");
    ...
    protected virtual void WriteSetAnnotations(Node node)
    {
        WriteLine();
    ...
    protected virtual void WriteSetDiagnostics(Node node)
    {
        WriteLine();
    ...
    protected virtual void WriteGreenVisitor(bool withResult)
    {
        var nodes = Tree.Types.Where(n => n is not PredefinedNode).ToList();
    ...
    protected virtual void WriteGreenUpdateMethod(Node node)
    {
        WriteLine();
    ...
    protected virtual void WriteGreenRewriter()
    {
        var nodes = Tree.Types.Where(n => n is not PredefinedNode).ToList();
    ...
    protected virtual void WriteContextualGreenFactories()
    {
    ...
    protected virtual void WriteStaticGreenFactories()
    {
        var nodes = Tree.Types.Where(n => n is not PredefinedNode and not AbstractNode).ToList();
        WriteLine();
    ...
    protected virtual void WriteGreenFactories(List<TreeType> nodes, bool withSyntaxFactoryContext = false)
    {
        foreach (var node in nodes.OfType<Node>())
        {
            WriteLine();
    ...
    protected virtual void WriteGreenTypeList()
    {
        WriteLine();
    ...
    protected virtual void WriteGreenFactory(Node nd, bool withSyntaxFactoryContext = false)
    {
        var valueFields = nd.Fields.Where(n => !IsNodeOrNodeList(n.Type)).ToList();
        var nodeFields = nd.Fields.Where(n => IsNodeOrNodeList(n.Type)).ToList();
    ...
    protected virtual void WriteGreenFactoryParameters(Node nd)
    {
        Write(CommaJoin(
    ...
    protected virtual void WriteCtorArgList(Node nd, bool withSyntaxFactoryContext, List<Field> valueFields, List<Field> nodeFields)
    {
        Write(CommaJoin(
    ...
    protected virtual void WriteRedTypes()
    {
        var nodes = Tree.Types.Where(n => n is not PredefinedNode).ToList();
        foreach (var node in nodes)
        {
    ...
    protected virtual List<Field> GetNodeOrNodeListFields(TreeType node)
    ...
    protected virtual void WriteRedType(TreeType node)
    {
        WriteComment(node.TypeComment, "");
    ...
    protected virtual string GetRedFieldType(Field field)
    {
    ...
    protected virtual string GetChildPosition(int i)
        => i == 0 ? "Position" : "GetChildPosition(" + i + ")";

    protected virtual string GetChildIndex(int i)
        => i == 0 ? "0" : "GetChildIndex(" + i + ")";

    protected virtual void WriteRedAcceptMethods(Node node)
    {
        WriteLine();
    ...
    protected virtual void WriteRedVisitor(bool genericResult)
    {
        string genericArgs = genericResult ? "<TResult>" : "";
        var nodes = Tree.Types.Where(n => n is not PredefinedNode).ToList();
    ...
    protected virtual void WriteRedUpdateMethod(Node node)
    {
        WriteLine();
        Write($"public {node.Name} Update(");
        Write(CommaJoin(
    ...
    protected virtual void WriteRedWithMethods(Node node)
    {
        foreach (var field in node.Fields)
        {
    ...
    protected virtual (TreeType type, Field field) GetHighestBaseTypeWithField(TreeType node, string name)
    {
    ...
    protected virtual TreeType TryGetBaseType(TreeType node)
        => node is AbstractNode an
            ? GetTreeType(an.Base)
            : node is Node n
                ? GetTreeType(n.Base)
    ...
    protected virtual void WriteRedListHelperMethods(Node node)
    {
    ...
    protected virtual Node TryGetNodeForNestedList(Field field)
    {
    ...
    protected virtual void WriteRedListHelperMethods(Node node, Field field)
    {
        var argType = GetElementType(field.Type);
    ...
    protected virtual void WriteRedNestedListHelperMethods(Node node, Field field, Node referencedNode, Field referencedNodeField)
    {
        var argType = GetElementType(referencedNodeField.Type);
    ...
    protected virtual void WriteRedRewriter()
    {
        var nodes = Tree.Types.Where(n => n is not PredefinedNode).ToList();
    ...
    protected virtual void WriteRedFactories()
    {
    ...
    protected virtual bool IsAutoCreatableToken(Node node, Field field)
    {
    ...
    protected virtual bool IsAutoCreatableNode(Field field)
    {
        var referencedNode = GetNode(field.Type);
        return (referencedNode != null && RequiredFactoryArgumentCount(referencedNode) == 0);
    }

    protected virtual bool IsRequiredFactoryField(Node node, Field field)
    {
        return (!IsOptional(field) && !IsAnyList(field.Type) && !CanBeAutoCreated(node, field)) || IsValueField(field);
    }

    protected virtual bool IsValueField(Field field)
    {
        return !IsNodeOrNodeList(field.Type);
    }

    protected virtual int RequiredFactoryArgumentCount(Node nd, bool includeKind = true)
    {
        int count = 0;
    ...
    protected virtual int OptionalFactoryArgumentCount(Node nd)
    {
        int count = 0;
        foreach (var field in nd.Fields)
        {
    ...
    protected virtual void WriteRedFactory(Node nd)
    {
        this.WriteLine();
    ...
    protected virtual void WriteRedFactoryParameters(Node nd)
    {
        Write(CommaJoin(
            nd.Kinds.Count > 1 ? "SyntaxKind kind" : "",
    ...
    protected virtual string GetRedPropertyType(Field field)
    {
        if (field.Type == "SyntaxList<SyntaxToken>")
            return "SyntaxTokenList";
        if (IsOptional(field) && IsNode(field.Type) && field.Type != "SyntaxToken")
    ...
    protected virtual string GetDefaultValue(Node nd, Field field)
    {
    ...
    protected virtual void WriteKindConverters(Node nd)
    {
        foreach (var field in nd.Fields)
        {
    ...
    protected virtual IEnumerable<Field> DetermineRedFactoryWithNoAutoCreatableTokenFields(Node nd)
    {
        return nd.Fields.Where(f => !IsAutoCreatableToken(nd, f));
    }

    // creates a factory without auto-creatable token arguments
    protected virtual void WriteRedFactoryWithNoAutoCreatableTokens(Node nd)
    {
    ...
    protected virtual Field DetermineMinimalOptionalField(Node nd)
    {
    ...
    protected static bool IsAttributeOrModifiersList(Field f) => f.Name is "AttributeLists" or "Modifiers";

    protected virtual IEnumerable<Field> DetermineMinimalFactoryFields(Node nd)
    {
    ...
    protected virtual void WriteRedMinimalFactory(Node nd, bool withStringNames = false)
    {
        var optionalCount = OptionalFactoryArgumentCount(nd);
        if (optionalCount == 0)
    ...
    protected virtual bool CanAutoConvertFromString(Field field)
    {
        return IsIdentifierToken(field) || IsIdentifierNameSyntax(field);
    }

    protected virtual bool IsIdentifierToken(Field field)
    {
        return field.Type == "SyntaxToken" && field.Kinds != null && field.Kinds.Count == 1 && field.Kinds[0].Name == "IdentifierToken";
    }

    protected virtual bool IsIdentifierNameSyntax(Field field)
    {
        return field.Type == "IdentifierNameSyntax";
    }

    protected virtual string GetStringConverterMethod(Field field)
    {
    ...
    /// XML doc comment.  Line breaks will be preserved.
    /// </summary>
    protected virtual void WriteComment(Comment comment, string indent)
    {
        if (comment != null)
        {
